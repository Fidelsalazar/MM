...
let Supabase = Supabase_1 = class Supabase {
    ...
    getClient() {
        ...
        this.clientInstance = (0, supabase_js_1.createClient)(this.configService.get('SUPABASE_URL'), this.configService.get('SUPABASE_KEY'));
        const token = passport_jwt_1.ExtractJwt.fromAuthHeaderAsBearerToken()(this.request);
        if (token) {
            this.clientInstance.auth.setSession({ access_token: token, refresh_token: '' });
